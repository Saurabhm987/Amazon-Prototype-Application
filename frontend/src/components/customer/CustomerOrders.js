...
class CustomerOrders extends Component {
    ...
    render() {
        const orders = this.props.order.userOrders.filter(order => { return(this.state.activeItem === 'Active' ? 
            order.status.status.indexOf('Cancelled') === -1 : order.status.status.indexOf('Cancelled') !== -1)
        })
